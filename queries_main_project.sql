...
CALL FindBuildingDetails('Hall');

-- vii. Get a list of all programs available in a specific department along with the number of credits required for completion in each program.
DELIMITER $$
CREATE PROCEDURE FindProgramsByDepartment(IN department VARCHAR(100))
 ...
 CALL FindProgramsByDepartment('Computer Science and Engineering');
...
CREATE PROCEDURE FindCoursesByTermAndProgram(IN season VARCHAR(100), IN year INT, IN program VARCHAR(100))
 ...
 CALL FindCoursesByTermAndProgram('SUMMER', 2019, 'Computer Science Thesis');
...
CALL FindCourseDetails('Computer Science and Engineering', 'Summer', 2019); 
...
CALL FindStudentsByProgramAndTerm('SUMMER', 2019, 'Computer Science Thesis');
...
CREATE PROCEDURE FindInstructorCourseByTerm(IN season VARCHAR(100), IN year INT, IN courseName VARCHAR(100), IN code VARCHAR(100), IN number INT)
 ...
 CALL FindInstructorCourseByTerm('SUMMER', 2019, 'Databases', 'COMP', 353);
...
CALL SelectAllSupervisorsInDepartment('Computer');
...
CALL SelectAllAdvisorsInDepartment('Computer Science');
...
CREATE PROCEDURE FindAllGraduateStudentsBySupervisor(IN professor VARCHAR(100))
 ...
 CALL FindAllGraduateStudentsBySupervisor('Brendan');
...
CREATE PROCEDURE FindAllTeachingAssistantAssignmentsByCourseAndTerm(IN course VARCHAR(100), IN code INT, IN season VARCHAR(100), IN year INT)
 ...
 CALL FindAllTeachingAssistantAssignmentsByCourseAndTerm('COMP', 353, 'SUMMER', 2019); 
...
-- xvii. For each department, find the total number of courses offered by the department and the name of its chairman
SELECT
	Department.name AS department,
    COUNT(Course.department_id) AS total,
    CONCAT(Person.first_name, ' ', Person.last_name) AS chairman
FROM Course
	INNER JOIN Department ON Course.department_id = Department.id
    INNER JOIN Person ON Department.chairman_id = Person.id
GROUP BY Course.department_id;

-- xviii. For each program, find the total number of students enrolled into the program
SELECT
	Program.name AS program,
    COUNT(StudentProgram.person_id) AS num_students_enrolled
FROM StudentProgram
	INNER JOIN Program ON StudentProgram.program_id = Program.id
GROUP BY StudentProgram.program_id;
...
CALL FindCoursesListByStudentAndTerm('Summer', 2019, 'Odafin', 'Speers');
...
CALL RegisterStudentInCourse(65, 16);
...
CREATE PROCEDURE GetStudentReport(IN firstName CHAR(100), IN lastName CHAR(100))
 ...
 CALL GetStudentReport('Odafin', 'Speers');